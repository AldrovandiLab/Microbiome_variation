...
pairs <- ddply(mapping.sel, .(SampleType, RunDateString), function(x) {
	t(combn(x$SampleID, 2))
}); colnames(pairs) <- c("SampleType", "RunDateString", "s1", "s2"); pairs$s1 <- as.character(pairs$s1); pairs$s2 <- as.character(pairs$s2)
pairs$st1 <- mapping.sel[pairs$s1, "SampleType2"]; pairs$st2 <- mapping.sel[pairs$s2, "SampleType2"]
pairs$Group <- factor(ifelse(pairs$st1 == "BacterialMock", "Mock-Within", ifelse(pairs$st1==pairs$st2, "Stool-Within", "Stool-Between")))
for (distance_metric in distance_metrics) {
	...
	test.1 <- wilcox.test(as.formula(sprintf("%s ~ %s", distance_metric, "Group")), subset(pairs, Group %in% c("Mock-Within", "Stool-Within")))
	test.2 <- wilcox.test(as.formula(sprintf("%s ~ %s", distance_metric, "Group")), subset(pairs, Group %in% c("Stool-Between", "Stool-Within")))
	df.annot <- aggregate(as.formula(sprintf("%s ~ %s", distance_metric, "Group")), pairs, mean)
	p <- ggplot(pairs, aes_string(x="Group", y=distance_metric)) + geom_boxplot() + theme_classic() + ggtitle(sprintf("%s by %s (%s p=%.4g)", distance_metric, "SampleType2", test$method, test$p.value)) + theme(title=element_text(size=8)) + annotate("text", x=2, y=0.1, label=sprintf("Mock-Within vs Stool-Within wilcoxon p=%.4g", test.1$p.value), size=4) + annotate("text", x=2, y=0.05, label=sprintf("Stool-Between vs Stool-Within wilcoxon p=%.4g", test.2$p.value), size=4) + geom_text(aes_string(x="Group", y=distance_metric, label=distance_metric), data=df.annot, inherit.aes=F)
	print(p)
}
```
...
<br><br>
Unsurprisingly, the 'Stool-Between' distances are significantly greater than the 'Stool-Within' distances; in other words, technical replicates of the same stool specimen are more closely related than biological replicates from the same donor. Interestingly, 'Mock-Within' distances are even smaller than the 'Stool-Within' distances, suggesting that technical variation likely depends on complexity of the community being studied (assumption here is that a real stool community is far more complex than our bacterial mock community).
...
pairs <- subset(pairs, WithinRun=="WithinRun")
pairs$Run <- mapping.sel[pairs$s1, "RunDateString"]
pairs <- subset(pairs, SampleType=="BacterialMock")
for (distance_metric in distance_metrics) {
	pairs[, distance_metric] <- dm[["nonblank"]][[distance_metric]][cbind(pairs$s1, pairs$s2)]
	test.mock <- kruskal.test(as.formula(sprintf("%s ~ %s", distance_metric, "Run")), pairs)
	p <- ggplot(pairs, aes_string(x="Run", y=distance_metric, fill="SampleType")) + geom_boxplot() + theme_classic() + ggtitle(sprintf("%s by %s (%s %.4g for BacterialMock, mean=%.4g)", distance_metric, "Run", test.mock$method, test.mock$p.value, mean(pairs[, distance_metric]))) + theme(title=element_text(size=8)) + scale_fill_brewer(palette="Set1")
	print(p)
}
```
...
```r
mean_ICC <- aggregate(ICC~taxlevel, res, mean)
mean_ICC
```
...
for (taxlevel in c("Family", "Genus", "Species", "SV")) {
	...
	heatmap.2(df, Colv=F, Rowv=F, dendrogram="none", col = cols, trace="none", margin=c(10, 20), cexCol=1.0, cexRow=0.6, labRow=rowlabels, adjRow=c(0,NA), density.info="none", key.xlab="CV", RowSideColors=mra.coloring, keysize=1, main=sprintf("CV heatmap - %s", taxlevel), scale="none", breaks=seq(from=0,to=5,by=0.1))
}
```

![Heatmap of taxa-specific coefficient of variation (CV) values over time. Greyscale shading on left indicates mean relative abundance of each taxon (also given in parentheses).](Microbiome_variation_files/figure-html/variation_over_time_cv-1.png)![Heatmap of taxa-specific coefficient of variation (CV) values over time. Greyscale shading on left indicates mean relative abundance of each taxon (also given in parentheses).](Microbiome_variation_files/figure-html/variation_over_time_cv-2.png)![Heatmap of taxa-specific coefficient of variation (CV) values over time. Greyscale shading on left indicates mean relative abundance of each taxon (also given in parentheses).](Microbiome_variation_files/figure-html/variation_over_time_cv-3.png)![Heatmap of taxa-specific coefficient of variation (CV) values over time. Greyscale shading on left indicates mean relative abundance of each taxon (also given in parentheses).](Microbiome_variation_files/figure-html/variation_over_time_cv-4.png)
