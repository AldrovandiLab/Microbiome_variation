...
```{r variation_over_time_taxa_barplot, eval=TRUE, warning=FALSE, message=FALSE, fig.cap="Taxa barplots for 117 bacterial mock samples sequenced over ~2 years. Samples are ordered chronologically by sequencing run and then by PC1."}
...
```{r variation_over_time_pairwise_distance, eval=TRUE, warning=FALSE, message=FALSE, fig.cap="Pairwise distances for 117 bacterial mock samples over time. Jenson-Shannon divergence (JSD) is shown."}
mapping.sel <- as(sample_data(psnonblank.relative), "data.frame")
mapping.sel$SampleID <- rownames(mapping.sel)
pairs <- ddply(mapping.sel, .(SampleType), function(x) {
	t(combn(x$SampleID, 2))
